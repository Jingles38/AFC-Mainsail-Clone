<template>
    <div class="afc-panel-wrapper">
        <panel :icon="mdiAdjust" :title="'AFC Spools'" card-class="afc-panel" :collapsible="true" :expanded="true">
            ...
            <!-- Multi Unit foldout -->
            <template v-if="Object.keys(unitsData).length > 1">
                <div v-for="(unit, unitName) in unitsData" :key="unitName" class="unit-section">
                    <v-expansion-panels>
                        <v-expansion-panel>
                            ...
                            <v-expansion-panel-content>
                                <div class="spool-container" :class="{'left-aligned': unit.spools.length <= 3}" style="margin-top: 15px">
                                    <div v-for="(spool, index) in unit.spools" :key="index" class="spool-card">
                                        <div
                                            v-if="spool.load"
                                            class="filament-reel"
                                            style="padding: 1rem">
                                            <spool-icon
                                                ...
                                                class="mr-3"
                                                @click-spool="openChangeSpoolDialog(spool)" />
                                            <FilamentReelIcon
                                                v-else
                                                :color="spool.color"
                                        ...
                                        <div class="spool-header">
                                            ...
                                            <select :name="'map-' + spool?.laneName" class="afclist" style="float: right;" @change="handleMapChange($event, spool)">
                                                <option :value="''" :selected="spool?.runout_lane === ''">NONE</option>
                                                <option
                                        ...
                                        <infinity-icon
                                            v-if="spool?.runout_lane === ''" color="#FF00000"
                                        />
                ...
                <div v-for="(unit, unitName) in unitsData" :key="unitName" class="unit-section">
                    ...
                    <div class="spool-container" :class="{'left-aligned': unit.spools.length <= 3}" style="margin-top: 15px">
                        <div v-for="(spool, index) in unit.spools" :key="index" class="spool-card">
                            ...
                            <select :name="'map-' + spool.laneName" class="afclist" @change="handleMapChange($event, spool)">
                                <option :value="''" :selected="spool?.runout_lane === ''">NONE</option>
                                <option
                                    v-for="option in laneList"
                                    :key="option"
                            ...
                            <infinity-icon
                                v-if="spool?.runout_lane?.isEmpty()" color="#FF00000"
                            />
                            <p v-if="spool?.material">{{ spool.material }}</p>
                            <p v-if="spool?.weight">{{ spoolWeight(spool) }}</p>
                        </div>
