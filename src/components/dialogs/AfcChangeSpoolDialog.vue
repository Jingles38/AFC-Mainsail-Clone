<template>
    <div>
        <v-dialog v-model="showDialog" width="800" persistent :fullscreen="isMobile">
            <panel :title="$t('Panels.SpoolmanPanel.ChangeSpool')"
                   :icon="mdiAdjust"
                ...
                <v-card-title>
                    ...
                    <v-spacer />
                    <v-btn :title="$t('Panels.SpoolmanPanel.EjectSpool')"
                           class="px-2 minwidth-0 ml-3"
                           :loading="loadings.includes('ejectSpool')"
                           @click="ejectSpool">
                        <v-icon>{{ mdiEject }}</v-icon>
                    </v-btn>
                    <v-btn :title="$t('Panels.SpoolmanPanel.Refresh')"
                           class="px-2 minwidth-0 ml-3"
                           :loading="loadings.includes('refreshSpools')"
                           @click="refreshSpools">
                        <v-icon>{{ mdiRefresh }}</v-icon>
                    </v-btn>
                    <v-btn :title="$t('Panels.SpoolmanPanel.OpenSpoolManager')"
                           class="px-2 minwidth-0 ml-3"
                ...
                <v-card-text class="px-0 pb-0">
                    <v-data-table :headers="headers"
                        ...
                        <template #item="{ item }">
                            <SpoolmanChangeSpoolDialogRow :key="item.id"
                                                          :spool="item"
